...
def on_message(ws, message):
    try:
        ...
        if code != 0:
            ...
            with open('./AI_CarAgent/answer.mp3', 'ab') as f:
                f.write(audio)
                #print("iconm")
...
def on_open(ws):
    def run(*args):
        ...
        #print("------>开始发送文本数据")
        ws.send(d)
        if os.path.exists('./AI_CarAgent/answer.mp3'):
            os.remove('./AI_CarAgent/answer.mp3')
...
def Car_Xinghou_speaktts(context="这个是合成音频程序"):
    ...
    #播放音频 PLAY AUDIO
    #proc = Popen("mplayer ./AI_CarAgent/answer.mp3 < /dev/null > /dev/null 2>1 &", shell=True)#sudo 
    #proc.wait()
    while is_mplayer_playing():#等待播放完成
        pass
    os.system('mplayer ./AI_CarAgent/answer.mp3 < /dev/null > /dev/null 2>1 &')
    time.sleep(0.5)

# Car_Xinghou_speaktts("测试测试")